# Advanced iOS Quiz — Week 1–12 + Git
**SmartFarmer Assistant · iOS 13+ · Swift + SwiftUI + CoreData**
...
| Q | Answer | Topic |
|---|--------|-------|
| 1 | B | Week 1 — @StateObject owns the ViewModel |
| 2 | B | Week 2 — NSPredicate filters at SQLite level |
| 3 | B | Week 2 — @NSManaged delegates storage to Obj-C runtime |
| 4 | B | Week 3 — NavigationLink(tag:selection:) for programmatic nav |
| 5 | B | Week 5 — requestAuthorization() before scheduling |
| 6 | B | Week 8 — @ViewBuilder allows multi-view closures |
| 7 | B | Week 9 — ViewModifier centralises styling |
| 8 | B | Week 10 — UIGraphicsPDFRenderer (iOS 10+) |
| 9 | C | Week 2 — saveContext() flushes to SQLite |
| 10 | B | Week 7 — context.coordinator is the delegate bridge |
| 11 | C | Week 12 — Any iOS Device (arm64) required for archive |
| 12 | B | Week 12 — OS shows launch screen before Swift process starts |
| 13 | D | Git — Working Directory → Staging Area → Repository |
| 14 | C | Git — git switch -c creates and switches in one step |
