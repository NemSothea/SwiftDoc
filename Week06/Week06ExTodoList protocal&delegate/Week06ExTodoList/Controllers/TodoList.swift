...
class TodoList: ObservableObject, TodoListManaging {
    
    @Published var todoItems: [TodoItem] = [] {
        didSet {
            saveTodos()
            delegate?.todoListDidUpdate(self)
        }
    }
    ...
    // MARK: - CRUD Operations with Error Handling
    func addTodo(title: String) {
        guard isTitleValid(title) else {
            delegate?.todoList(self, didEncounterError: .invalidTitle)
            return
        }
        
        guard !isDuplicateTitle(title) else {
            delegate?.todoList(self, didEncounterError: .duplicateTitle)
            return
        }
    ...
    func toggleTodoCompletion(_ todo: TodoItem) {
        guard let index = todoItems.firstIndex(where: { $0.id == todo.id }) else {
            delegate?.todoList(self, didEncounterError: .todoNotFound)
            return
        }
    ...
    func updateTodo(_ todo: TodoItem, newTitle: String) {
        guard isTitleValid(newTitle) else {
            delegate?.todoList(self, didEncounterError: .invalidTitle)
            return
        }
        ...
        let isDuplicate = todoItems.contains {
            $0.id != todo.id && $0.title.lowercased() == newTitle.lowercased()
        }
